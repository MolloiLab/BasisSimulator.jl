...
# NV×NV×2 grid (45 cm FOV), VIEWS views, RECON×RECON×2 recon; the pipeline sizes its compiled view
# loops from BUDGET_MB (view_batch = :auto) unless VB > 0 forces a batch size.
#   NV=128 VIEWS=200 RECON=128 BUDGET_MB=512 GRAD=1 julia --project=envs/reactant -t 2 --heap-size-hint=4G design/reactant/probes/scale_view_batch.jl
...
NV = envi("NV", 128); VIEWS = envi("VIEWS", 200); RECON = envi("RECON", 128); VB = envi("VB", 0); GRAD = envi("GRAD", 1); LEGACY = envi("LEGACY", 1)
BUDGET_MB = envi("BUDGET_MB", 512)
T0 = time(); say(m) = (println(@sprintf("[%7.1f s] ", time() - T0), m); flush(stdout))
say("config NV=$NV VIEWS=$VIEWS RECON=$RECON VB=$VB BUDGET_MB=$BUDGET_MB GRAD=$GRAD")
...
simo = BS.SimOptions(seed = 1234, projector = :dd_fast, use_noise = false, use_scatter = false,
...
t = @elapsed pipe = BSF.eict_pipeline(phantom, scanner, protocol, simo, recon_opts; view_batch = VB > 0 ? VB : :auto, batch_budget_mb = BUDGET_MB)
runs = BSF.dd_run_plans(pipe.geom, pipe.vol_shape; view_batch = pipe.batching.dd, volume_extent = pipe.volume_extent, eltype = Float32)
say(@sprintf("pipeline built in %.1f s: sino %s, batching %s, %d DD runs (nb/tail %s), taps KX·KZ max %d", t, string(pipe.eict.sino_shape),
    string(pipe.batching), length(runs), join(["$(r.nb)/$(r.tail)" for r in runs], ","), maximum(r.KX * r.KZ for r in runs)))
