...
# --- compile one forward and one pullback program per (orientation, length)
...
tcomp = @elapsed for (i, b) in enumerate(bs)
    ...
    f = (x, d) -> BSF.eict_batch_vol(x, pipe, b, d)
    g = (x, vbar, d) -> Enzyme.gradient(Reverse, (x, vbar, d) -> sum(vbar .* BSF.eict_batch_vol(x, pipe, b, d)), x, Const(vbar), Const(d))[1]
    vbar0 = Reactant.to_rarray(zeros(Float32, NV, NV, 2))
    fwd_c[k] = @compile sync = true f(fr_r, d)
    vjp_c[k] = @compile sync = true g(fr_r, vbar0, d)
end
say(@sprintf("compiled %d forward + %d pullback batch programs in %.0f s", length(fwd_c), length(vjp_c), tcomp))
tohu = @compile sync = true BSF.eict_vol_to_hu(Reactant.to_rarray(zeros(Float32, NV, NV, 2)), pipe)
# --- forward
function forward!(x)
    vol = nothing
    for (i, b) in enumerate(bs)
        v = fwd_c[key(b)](x, data_r[i])
        vol = vol === nothing ? v : (vol .+ v)     # host-side accumulate on device arrays
    end
    return vol
end
...
function gradient!(x)
    ...
    gsum = nothing
    for (i, b) in enumerate(bs)
        gb = vjp_c[key(b)](x, vb, data_r[i])
        gsum = gsum === nothing ? gb : (gsum .+ gb)
    end
    return gsum
end
