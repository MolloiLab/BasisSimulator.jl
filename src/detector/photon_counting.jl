...
- `flux_rate::Union{Nothing,Real}=nothing`: Photon flux rate [photons/s/mm²] for pileup.
  If `nothing` (default), auto-computed from `I0 / pixel_area` so that pileup
  severity scales correctly with mAs. Set explicitly to override.
...
function pcct_forward_project(
    ...
    flux_rate::Union{Nothing, Real} = nothing,
    I0::Real = 1e6,
    apply_spectral_response::Bool = true,
    apply_detector_effects::Bool = true,
    apply_corrections::Bool = false,
    use_unified_drm::Bool = false,
    # MC-simulated detector response (replaces analytical DRM)
    use_mc_drm::Bool = false,
    mc_drm_path::Union{Nothing, String} = nothing,
    # Monte Carlo pileup (replaces analytical Taguchi model)
    use_mc_pileup::Bool = false,
    mc_pileup_trials::Int = 10000,
    # Workspace buffers (optional — allocate internally if not provided)
    ws_bins = nothing,
    ws_μ_volume = nothing,
    ws_sino_buf = nothing,
    ...
    # Auto-compute flux_rate from I0 and pixel geometry if not explicitly provided.
    # This correctly couples mAs → I₀ → flux_rate → pileup severity.
    # Physics: flux_rate [photons/s/mm²] = I0 [photons/pixel] / pixel_area [mm²]
    # The pileup model then converts: count_rate = flux_rate × pixel_area = I0
    if flux_rate === nothing
        pixel_area_mm2 = detector.pixel_size_mm[1] * detector.pixel_size_mm[2]
        flux_rate = Float64(I0) / pixel_area_mm2
    end
    ...
    elseif apply_spectral_response
        if use_mc_drm
            # MC-based DRM: includes charge sharing, Fano noise, charge cloud,
            # pixel splitting, electronic noise (Stierstorfer 2018)
            compute_mc_drm(detector, kVp;
                npz_path=(mc_drm_path !== nothing ? mc_drm_path : default_mc_drm_path()),
                n_energy_points=n_energies)
        elseif use_unified_drm
    ...
    if apply_detector_effects
        # MC DRM already includes charge sharing (3×3 erf splitting) —
        # skip analytical model when MC DRM is active
        if !use_mc_drm
            apply_charge_sharing!(bins, detector; scratch=_scratch,
                                  ws_charge_probs=ws_charge_probs)
        end

        # Pileup: use MC-based or analytical model
        if use_mc_pileup
            apply_mc_pileup!(bins, detector, Float64(flux_rate),
                             weights, energies;
                             n_mc_trials=mc_pileup_trials,
                             seed=isnothing(detector.seed) ? 42 : detector.seed,
                             ws_mc_S=ws_mc_S)
        else
