...
# =============================================================================
#
# Public surface kept intentionally narrow: notebooks access Gammex 472
# materials via Symbol lookup (`get_material(:Ca_100)`), not via the
# `BS.Ca_100` binding name.  The `const Ca_*` / `const I_*` / `const
# solid_water` definitions and `MATERIALS_REGISTRY` are kept module-internal
# (no `export`) so the public API doesn't grow with every Gammex insert.
