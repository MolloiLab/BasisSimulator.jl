...
"""
    _combine_pcct_bins(pcct_sino, detector, energies, weights, kVp; I0=1e6,
                        apply_detector_effects=false, apply_corrections=false, flux_rate=nothing)
...
function _combine_pcct_bins(pcct_sino::EnergyResolvedSinogram, detector::PhotonCountingDetector,
    ...
    elseif apply_detector_effects && !apply_corrections
        ...
        # Auto-compute flux_rate from I0 if needed
        _flux_rate = if flux_rate === nothing
            pixel_area_mm2 = detector.pixel_size_mm[1] * detector.pixel_size_mm[2]
            Float64(I0) / pixel_area_mm2
        else
            Float64(flux_rate)
        end
        _compute_degraded_I0(detector, energies, weights, η, thresholds, kVp, I0, _flux_rate; R=R)
    else
