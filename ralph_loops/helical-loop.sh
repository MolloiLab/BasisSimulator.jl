...
# BasisSimulator.jl Helical CT Discovery Loop
#
...
# Output: helical-spec.md (the definitive spec for adding helical CT scanning).
#
# Usage:
#   ./ralph_loops/helical-loop.sh          # Default 30 iterations
#   ./ralph_loops/helical-loop.sh 15       # Custom max iterations
...
PROMPT_FILE="$RALPH_DIR/helical-prompt.md"
STATE_FILE="$RALPH_DIR/helical-state.md"
SPEC_FILE="$RALPH_DIR/helical-spec.md"
PROGRESS_FILE="$RALPH_DIR/helical-progress.md"
PRD_FILE="$RALPH_DIR/helical-prd.json"

# Exit markers
COMPLETE_MARKER="HELI_COMPLETE"
BLOCKED_MARKER="HELI_BLOCKED"
...
echo ""
echo -e "${CYAN}╔════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${NC}  ${BOLD}BasisSimulator.jl — Helical CT Discovery Loop${NC}              ${CYAN}║${NC}"
echo -e "${CYAN}║${NC}  Research → Critique → Refine → Spec                         ${CYAN}║${NC}"
echo -e "${CYAN}║${NC}  Goal: Seamless helical scan integration                     ${CYAN}║${NC}"
echo -e "${CYAN}╚════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
while [ $iteration -lt $MAX_ITERATIONS ]; do
    ...
    if [ "$OPEN" -eq 0 ]; then
        echo -e "${GREEN}══════════════════════════════════════════════════════════════${NC}"
        echo -e "${GREEN}  All research complete! Helical spec is ready at:${NC}"
    ...
    START_TIME=$(date +%s)
    TEMP_OUTPUT="/tmp/helical_discovery_ralph_$$"
    ...
    while ps -p $CLAUDE_PID > /dev/null 2>&1; do
        ...
        if [ $ELAPSED -ge $TIMEOUT ]; then
            ...
            if [ -n "$(git status --porcelain ralph_loops/helical-* 2>/dev/null)" ]; then
                echo -e "  Saving uncommitted research..."
                git add ralph_loops/helical-*.md ralph_loops/helical-*.json 2>/dev/null || true
                git commit -m "TIMEOUT-SAVE: Helical discovery iteration $iteration" 2>/dev/null || true
            fi
    ...
    if echo "$OUTPUT" | grep -q "$BLOCKED_MARKER"; then
        echo ""
        echo -e "${RED}  Research blocked. Check helical-state.md for details.${NC}"
        exit 1
    fi
