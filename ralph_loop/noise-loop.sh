...
# BasisSimulator — Noise 2x Diagnosis Ralph Loop
#
# Runs Claude Code in a loop. Each iteration is a FRESH agent with zero memory.
# State persists via: noise-prd.json (stories), noise-progress.md (log), git (code).
#
# Investigation: BasisSimulator produces ~2x more noise than CatSim/XCIST
# for identical CT simulation parameters (120 kVp, 200 mA, 984 views).
#
# Usage:
#   ./ralph_loop/noise-loop.sh          # Default 30 iterations
#   ./ralph_loop/noise-loop.sh 10       # Custom max iterations
...
PRD_FILE="$RALPH_DIR/noise-prd.json"
PROGRESS_FILE="$RALPH_DIR/noise-progress.md"
PROMPT_FILE="$RALPH_DIR/noise-prompt.md"
CONTEXT_FILE="$RALPH_DIR/noise-context.md"
GUARDRAILS_FILE="$RALPH_DIR/noise-guardrails.md"
LOG_FILE="$RALPH_DIR/noise-loop-log.txt"
...
" 2>/dev/null || echo "")
        ...
        osascript -e "display notification \"$new_stories\" with title \"Noise Diagnosis\" subtitle \"$count story(ies) completed\"" 2>/dev/null || true
    fi
}
...
auto_commit_on_timeout() {
    ...
    if [ "$has_changes" = true ]; then
        ...
        git add ralph_loop/noise-progress.md ralph_loop/noise-prd.json 2>/dev/null || true
        git add ralph_loop/scripts/ 2>/dev/null || true
        git commit -m "[TIMEOUT] Auto-save noise diagnosis progress" 2>/dev/null || true
...
echo ""
echo -e "${BLUE}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║${NC}  ${BOLD}BasisSimulator — Noise 2x Diagnosis${NC}                     ${BLUE}║${NC}"
echo -e "${BLUE}║${NC}  Max iterations: $MAX_ITERATIONS                                       ${BLUE}║${NC}"
echo -e "${BLUE}║${NC}  Target: Find why noise is 2x worse than CatSim           ${BLUE}║${NC}"
echo -e "${BLUE}╚═══════════════════════════════════════════════════════════════╝${NC}"
print_status
echo ""
log_msg "Noise diagnosis loop started. Max iterations: $MAX_ITERATIONS"
...
while [ $iteration -lt $MAX_ITERATIONS ]; do
    ...
    DONE_BEFORE="/tmp/ralph_noise_done_before_$$_${iteration}"
...
"
    ...
    START_TIME=$(date +%s)
    TEMP_OUTPUT="/tmp/ralph_noise_output_$$_${iteration}"
    ...
    if echo "$OUTPUT" | grep -q "$COMPLETE_MARKER"; then
        echo ""
        echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${GREEN}║  RALPH_COMPLETE — Noise root cause identified and fixed!     ║${NC}"
        echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    ...
    if echo "$OUTPUT" | grep -q "$BLOCKED_MARKER"; then
        echo ""
        echo -e "${RED}Agent is blocked. Check noise-progress.md for details.${NC}"
