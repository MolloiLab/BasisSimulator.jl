...
auto_commit_on_timeout() {
    ...
    if [ "$has_changes" = true ]; then
        ...
        git add ralph_loop/ src/ 2>/dev/null || true
        git commit -m "[TIMEOUT] Auto-save BHC implementation progress" 2>/dev/null || true
...
echo ""
echo -e "${BLUE}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║${NC}  ${BOLD}BasisSimulator — Two-Material BHC${NC}                      ${BLUE}║${NC}"
...
while [ $iteration -lt $MAX_ITERATIONS ]; do
    ...
    if echo "$OUTPUT" | grep -q "$COMPLETE_MARKER"; then
        echo ""
        echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${GREEN}║  RALPH_COMPLETE — Two-material BHC implemented!              ║${NC}"
        echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
