...
#
# Ralph loop: runs Claude Code in an infinite loop where each iteration is a
# FRESH agent with zero memory. State persists via files + git only.
#
# Principles:
#   - Each iteration = fresh agent, no context from previous runs
#   - State lives in: prd.json (stories), progress.md (log), git (code)
#   - Agent reads state → picks highest-priority unblocked story → executes → commits
#   - Loop detects RALPH_COMPLETE or RALPH_BLOCKED to exit
#
# Usage:
#   ./ralph_loop/loop.sh          # Default 30 iterations
#   ./ralph_loop/loop.sh 10       # Custom max iterations
...
# Timeout per iteration (60 minutes — GPU CT simulations + JIT compilation need time)
MAX_WAIT=3600
...
" 2>/dev/null || echo "")
        ...
        osascript -e "display notification \"$new_stories\" with title \"Ralph Loop\" subtitle \"$count story(ies) completed\"" 2>/dev/null || true
    fi
}
...
auto_commit_on_timeout() {
    ...
    if ! git diff --quiet ralph_loop/ 2>/dev/null; then
        has_changes=true
    fi
    if ! git diff --cached --quiet ralph_loop/ 2>/dev/null; then
        has_changes=true
    fi
    if ! git diff --quiet verification/ 2>/dev/null; then
        has_changes=true
    fi

    if [ "$has_changes" = true ]; then
        echo -e "${YELLOW}  Auto-committing changes on timeout...${NC}"
        git add ralph_loop/ verification/ 2>/dev/null || true
        git commit -m "[TIMEOUT] Auto-save verification migration progress" 2>/dev/null || true
...
echo ""
echo -e "${BLUE}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║${NC}  ${BOLD}BasisSimulator — Verification Suite Migration${NC}          ${BLUE}║${NC}"
...
while [ $iteration -lt $MAX_ITERATIONS ]; do
    ...
    DONE_BEFORE="/tmp/ralph_verif_done_before_$$_${iteration}"
    get_done_stories > "$DONE_BEFORE"

    # Run Claude Code with fresh context
    PROMPT_CONTENT=$(cat "$PROMPT_FILE")
    START_TIME=$(date +%s)
    TEMP_OUTPUT="/tmp/ralph_verif_output_$$_${iteration}"
    ...
    if echo "$OUTPUT" | grep -q "$COMPLETE_MARKER"; then
        echo ""
        echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${GREEN}║  RALPH_COMPLETE — Verification suite migration done!         ║${NC}"
        echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
