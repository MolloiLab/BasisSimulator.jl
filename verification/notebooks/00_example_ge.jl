...
let
	...
	CM.lines!(ax, e_raw, w_raw ./ maximum(w_raw);
		color = :gray60, linewidth = 1.5, label = "Raw tube (no filter)")
	CM.lines!(ax, e_raw, w_filt ./ maximum(w_filt);
		color = :steelblue, linewidth = 2, label = "After 7.0 mm Al")
	CM.lines!(ax, e_res, w_res ./ maximum(w_res);
		color = :darkorange, linewidth = 2, label = "Resolved (+ bowtie + detector)")
	CM.vlines!(ax, [E_eff]; color = :red, linestyle = :dash, linewidth = 1,
...
let
	...
	ax1 = CM.Axis(fig[1, 1]; title = "Sinogram — Mid-view (view $mid_view)",
		xlabel = "Detector column", ylabel = "Detector row")
	CM.heatmap!(ax1, sim_sino.sino[:, mid_view, :]'; colormap = :grays)

	ax2 = CM.Axis(fig[1, 2]; title = "Sinogram — Mid-row (row $mid_row)",
...
let
	...
	fbp_vals = [sim_fbp_hu[cx+dx, cy+dy, mid_z]
		for dy in -r:r for dx in -r:r if dx^2 + dy^2 <= r^2]
	hir_vals = [sim_hir_hu[cx+dx, cy+dy, mid_z]
		for dy in -r:r for dx in -r:r if dx^2 + dy^2 <= r^2]
	...
	ax = CM.Axis(fig[1, 1]; title = "Water ROI Noise",
		ylabel = "Water σ (HU)", xticks = ([1, 2], ["FBP", "HIR"]))
	CM.barplot!(ax, [1], [fbp_std]; width = 0.5, color = :steelblue,
		label = "FBP: μ=$(round(fbp_mean, digits=1)), σ=$(round(fbp_std, digits=1))")
	CM.barplot!(ax, [2], [hir_std]; width = 0.5, color = :darkorange,
...
let
	...
	fig = CM.Figure(size = (900, 400), fontsize = 13)
	ax = CM.Axis(fig[1, 1]; title = "Horizontal Line Profile (mid-slice)",
		xlabel = "Position (cm)", ylabel = "HU")
	CM.lines!(ax, collect(x_cm), Float64.(fbp_profile);
		color = :steelblue, linewidth = 1.5, label = "FBP")
	CM.lines!(ax, collect(x_cm), Float64.(hir_profile);
