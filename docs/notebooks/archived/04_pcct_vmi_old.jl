...
# ╔═╡ 040c0001-0000-4000-8000-000000000001
begin
    import Pkg
    Pkg.activate(joinpath(@__DIR__, ".."))
end

# ╔═╡ 040c0001-0000-4000-8000-000000000002
using Markdown: @md_str, Markdown

# ╔═╡ 040c0001-0000-4000-8000-000000000003
using Statistics: mean, std

# ╔═╡ 040c0001-0000-4000-8000-000000000010
md"""
...
# ╔═╡ 040c0001-0000-4000-8000-000000000020
md"""
## Setup
...
# ╔═╡ 040c0001-0000-4000-8000-000000000030
import BasisSimulator as BS

# ╔═╡ 040c0001-0000-4000-8000-000000000031
import CairoMakie as CM

# ╔═╡ 040c0001-0000-4000-8000-000000000040
begin
    GPU_BACKEND = let
        candidates = [
...
# ╔═╡ 040c0001-0000-4000-8000-000000000050
md"""
**Backend detected:** $(GPU_BACKEND.name)
"""

# ╔═╡ 040c0002-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c0002-0000-4000-8000-000000000010
...
# ╔═╡ 040c0002-0000-4000-8000-000000000020
phantom = BS.Phantom(
...
# ╔═╡ 040c0003-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c0003-0000-4000-8000-000000000010
scanner = let
...
# ╔═╡ 040c0004-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c0004-0000-4000-8000-000000000010
...
# ╔═╡ 040c0005-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c0005-0000-4000-8000-000000000010
...
# ╔═╡ 040c0005-0000-4000-8000-000000000020
recon_opts = let
...
# ╔═╡ 040c0007-0000-4000-8000-000000000001
md"""
## 5. PCCT simulation
...
# ╔═╡ 040c0008-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c0009-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c0009-0000-4000-8000-000000000010
sim_lohi = let
...
# ╔═╡ 040c000a-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c000a-0000-4000-8000-000000000010
de_lohi_μ = let
...
# ╔═╡ 040c000b-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c000b-0000-4000-8000-000000000005
rskr_knob = (
    n_iter = 2,
    h_param = 2,
    radius = 3,
...
# ╔═╡ 040c000b-0000-4000-8000-000000000007
...
# ╔═╡ 040c000b-0000-4000-8000-000000000010
de_lohi_rskr = let
    out = BS.apply_rskr(
...
# ╔═╡ 040c000b-0000-4000-8000-000000000020
md"""
...
# ╔═╡ 040c000b-0000-4000-8000-000000000030
let
...
# ╔═╡ 040c0006-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c0006-0000-4000-8000-000000000005
...
# ╔═╡ 040c0006-0000-4000-8000-000000000010
...
# ╔═╡ 040c0006-0000-4000-8000-000000000012
...
# ╔═╡ 040c0006-0000-4000-8000-000000000014
...
# ╔═╡ 040c0006-0000-4000-8000-000000000016
...
# ╔═╡ 040c0006-0000-4000-8000-000000000018
μ_water_per_bin = let
    src = μ_water_source
    if src === :analytic
...
# ╔═╡ 040c0006-0000-4000-8000-000000000020
md"""
### μ_water comparison
...
# ╔═╡ 040c000c-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c000c-0000-4000-8000-000000000010
de_lohi_HU = let
...
# ╔═╡ 040c000d-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c000d-0000-4000-8000-000000000010
ROD_LABELS = (
...
# ╔═╡ 040c000d-0000-4000-8000-000000000020
ROD_NAMES = (
...
# ╔═╡ 040c000d-0000-4000-8000-000000000050
rod_rois = let
...
# ╔═╡ 040c000d-0000-4000-8000-000000000060
...
# ╔═╡ 040c000d-0000-4000-8000-000000000063
md"""
## 12. Calibration
...
# ╔═╡ 040c000d-0000-4000-8000-000000000074
...
# ╔═╡ 040c000d-0000-4000-8000-000000000080
let
    body = """
...
# ╔═╡ 040c000e-0000-4000-8000-000000000001


# ╔═╡ 040c000e-0000-4000-8000-000000000010
de_decomp = let
...
# ╔═╡ 040c000f-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c000f-0000-4000-8000-000000000010
de_vmi_energies = [40.0, 70.0, 100.0, 140.0];

# ╔═╡ 040c000f-0000-4000-8000-000000000020
de_vmi_raw = let
...
# ╔═╡ 040c000f-0000-4000-8000-000000000030
md"""
## 13.5. Per-VMI radial cupping correction (HU-space, post-synth · pre-Mono+)

[`BS.apply_radial_cupping_correction!`](@ref) flattens any residual
radial cup/halo on each VMI volume *before* Mono+ sees it.  Sino-domain
artifacts (residual scatter, RSKR halo, decomp-induced cup) all
project into the synth output as a slow radial bias; an even-poly
fit on water-like background voxels mops it up slice-by-slice.

PCCT VMIs are tighter on water than dual-kVp (no
beam-hardening-vs-iodine-K-edge tug-of-war), so the default HU
window is much narrower — `(-20, 20)` keeps the polynomial fit on
truly-water voxels and avoids pulling soft-tissue or low-c iodine
samples into the background fit.

| Knob          | Default      | Notes                                                        |
|---------------|--------------|--------------------------------------------------------------|
| `enabled`     | `true`       | flip to bypass entirely (sets `de_vmi` ≡ `de_vmi_raw`)       |
| `fov_cm`      | `35.0`       | in-plane FOV; pixel scale = fov_cm / nx                      |
| `hu_lo`       | `-20.0`      | lower threshold for "water-like" sample voxels (PCCT-tight)  |
| `hu_hi`       | `20.0`       | upper threshold (excludes Ca + I rods + soft tissue)         |
| `poly_order`  | `3`          | even-poly terms (1=parabolic, 2=+r⁴, 3=+r⁶ stiffer)          |
| `target_hu`   | `0.0`        | target HU for the water background after correction          |
"""
...
# ╔═╡ 040c0010-0000-4000-8000-000000000001


# ╔═╡ 040c0010-0000-4000-8000-000000000010
de_vmi_mono = let
    vols_in = [de_vmi[E] for E in de_vmi_energies]   # cupping-corrected VMIs from §13.5
...
# ╔═╡ 040c0011-0000-4000-8000-000000000001


# ╔═╡ 040c0011-0000-4000-8000-000000000010
let
    ...
    CM.save(
        joinpath(@__DIR__, "..", "assets", "pcct_vmi_4panel_old.png"),
        fig; px_per_unit = 2,
    )
    fig
end

# ╔═╡ 040c0012-0000-4000-8000-000000000001


# ╔═╡ 040c0012-0000-4000-8000-000000000010
rod_data = let
...
# ╔═╡ 040c0012-0000-4000-8000-000000000020
...
# ╔═╡ 040c0013-0000-4000-8000-000000000001


# ╔═╡ 040c0013-0000-4000-8000-000000000010
let
    ...
    CM.save(
        joinpath(@__DIR__, "..", "assets", "pcct_vmi_vs_theoretical_old.png"),
        fig; px_per_unit = 2,
    )
    fig
end

# ╔═╡ 040c0014-0000-4000-8000-000000000001
...
# ╔═╡ 040c0014-0000-4000-8000-000000000010
let
    ...
    CM.save(
        joinpath(@__DIR__, "..", "assets", "pcct_vmi_regression_old.png"),
        fig; px_per_unit = 2,
    )
    fig
end

# ╔═╡ 040c0015-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c0015-0000-4000-8000-000000000010
let
...
# ╔═╡ 040c0016-0000-4000-8000-000000000001
md"""
...
# ╔═╡ 040c0016-0000-4000-8000-000000000010
let
...
# ╔═╡ 040c0017-0000-4000-8000-000000000001
md"""
## Summary
