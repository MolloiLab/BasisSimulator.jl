...
md"""
# Fixed Four-Bin Cong and Total-Likelihood Bilateral Filtering
...
This notebook starts from the now-fixed method:

`four corrected PCCT bins → profiled Cong → raw Lee T-LBF → common FBP → VMI`.

Method-development experiments remain in `04d_pcct_vmi.jl` and Git history.
"""
...
pcct_results = let
    ...
    μwater=Dict(
        ...
        for energy in pcct_tlbf.energies
    )
    rods=Dict{Symbol,NamedTuple}()
    for group in (:Ca,:I)
        measured=zeros(length(rod_labels[group]),length(pcct_tlbf.energies))
        theoretical=similar(measured)
        for (row,label) in pairs(rod_labels[group])
            roi=rod_roi(label)
            material=phantom_cpu.materials[Int(label)+1]
            for (column,energy) in pairs(pcct_tlbf.energies)
                measured[row,column]=mean(pcct_tlbf.vmis[:,:,column][roi])
    ...
    (
        water_mask,rods,
        water_mean=[
            mean(pcct_tlbf.vmis[:,:,index][water_mask])
            for index in axes(pcct_tlbf.vmis,3)
        ],
        water_noise=[
            std(pcct_tlbf.vmis[:,:,index][water_mask])
            for index in axes(pcct_tlbf.vmis,3)
        ],
        finite=all(isfinite,pcct_tlbf.vmis),
...
let
    ...
    panels=(
        ("Iodine Basis","g/cm³",pcct_tlbf.images.iodine),
        ("Water Basis","g/cm³",pcct_tlbf.images.water),
    )
...
let
    ...
    for (index,energy) in pairs(pcct_tlbf.energies)
        row=((index-1)÷2)+1
        column=((index-1)%2)+1
        axis=Mke.Axis(
...
let
    ...
    index70=findfirst(==(70.0),pcct_tlbf.energies)
    image70=pcct_tlbf.vmis[:,:,index70]
    n=length(pcct_tlbf.energies)
    fig=Mke.Figure(size=(1180,1180))
    for row in (1,2)
        axis=Mke.Axis(
            fig[row,1];
            title=row==1 ? "Eroded Water Region" : "Water Noise Region",
    ...
    mean_axis=Mke.Axis(
        ...
        xticks=(1:n,string.(Int.(pcct_tlbf.energies))),titlesize=32,
    )
    ...
    noise_axis=Mke.Axis(
        fig[2,2];title="Water-Region Noise vs Energy",
...
let
    ...
    for (column,panel) in pairs(panels)
        axis=Mke.Axis(
            ...
            xlabel="VMI energy (keV)",ylabel="HU",
            xticks=pcct_tlbf.energies,titlesize=32,subtitlesize=24,
        )
        ...
        for index in eachindex(data.names)
            ...
            Mke.scatterlines!(
                axis,pcct_tlbf.energies,vec(data.measured[index,:]);
                color,linewidth=2.5,markersize=9,label=data.names[index],
            )
            Mke.lines!(
                axis,pcct_tlbf.energies,vec(data.theoretical[index,:]);
...
`four corrected native PCCT bins → all-bin profiled Cong → joint Lee T-LBF
(5×5, α₁=0.9, α₂=$(round(pcct_tlbf.alpha2,digits=3))) → identical SoftFilter
FBP → analytical VMI`

The total count enters only the T-LBF weight calculation. Both completed
material sinograms receive the same normalized neighborhood weight. The
collapsed total count is never used for material estimation.
"""

# ╔═╡ 040e0002-0000-4000-8000-000000000001
md"""
## Literature-grounded monotonic-noise extension

The fixed Cong and raw T-LBF stages above remain unchanged. The first
post-reconstruction test is Kalender anti-correlated noise reduction (ACNR),
applied jointly to the two FBP material maps before VMI synthesis.

Kalender, Klotz, and Kostaridou showed that basis-material noise is negatively
correlated and can be suppressed while constraining monoenergetic CT values;
they reported two- to five-fold noise reduction, with local adaptation needed
near edges ([IEEE TMI, 1988](https://doi.org/10.1109/42.7785)).
...
acnr_comparison = let
    energies=collect(40.0:5.0:190.0)
    configurations=(
        (name="T-LBF only",passes=0,beta_max=0.0),
        (name="Kalender ACNR default",passes=2,beta_max=8.0),
        (name="Kalender ACNR expanded",passes=4,beta_max=20.0),
        (name="Kalender ACNR 6-pass",passes=6,beta_max=20.0),
        (name="Kalender ACNR 8-pass",passes=8,beta_max=20.0),
    )
    entries=NamedTuple[]
    elapsed=@elapsed for configuration in configurations
        water=copy(pcct_tlbf.images.water)
        iodine=copy(pcct_tlbf.images.iodine)
        info=configuration.passes==0 ? nothing :
            BS.apply_acnr_kalender!(
                water,iodine;
                hp_sigma_px=1.5,window=4,
                passes=configuration.passes,
                beta_max=configuration.beta_max,
            )
        vmis=synthesize_vmi_stack((;water,iodine),energies)
        noise=[
            std(vmis[:,:,index][pcct_results.water_mask])
            for index in axes(vmis,3)
        ]
        means=[
            mean(vmis[:,:,index][pcct_results.water_mask])
            for index in axes(vmis,3)
        ]
        push!(entries,(
...
let
    ...
    axis=Mke.Axis(
        fig[1,1];title="ACNR effect on VMI noise",
        subtitle="Fixed four-bin Cong + raw T-LBF",
        xlabel="VMI energy (keV)",ylabel="Water-region noise σ (HU)",
        titlesize=30,subtitlesize=20,
    )
    for entry in acnr_comparison.entries
        Mke.scatterlines!(
            axis,acnr_comparison.energies,entry.noise;
    ...
    for (row,(label,stack)) in pairs(rows)
        for (column,energy) in pairs(energies_to_show)
            ...
            axis=Mke.Axis(
                ...
                ylabel=column==1 ? label : "",
                aspect=Mke.DataAspect(),titlesize=28,ylabelsize=24,
            )
            Mke.heatmap!(
                axis,stack[:,:,index];
                colormap=:grays,colorrange=(-200,500),
            )
            Mke.hidedecorations!(axis;label=false)
        end
    end
    Mke.Colorbar(
