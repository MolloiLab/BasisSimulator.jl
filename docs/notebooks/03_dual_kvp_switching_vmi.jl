...
```
Simulate 80 kVp  →┐
                   ├─→  Joint Sinogram Denoising  (2 channels)
Simulate 140 kVp →┘                  │
                                     ▼
...
!!! info "Why Projection Domain?"
    ...
    2. **Joint denoising before reconstruction.** A per-detector-row
       SVD denoiser exploits the strong cross-kVp correlation
       (same view angles, geometry, anatomy) and strips quantum noise
       before it can propagate through decomposition + FBP.
...
The 80 + 140 kVp sinograms share the same anatomy (same view angles,
geometry, Radon transform) but differ in amplitude, material-specific
ratio, and quantum noise.  `BS.apply_sino_svd_denoise` exploits that
correlation.

!!! info "Algorithm"
    Per detector row, stack the channels as columns of a 2-wide
...
SINO_DENOISE_σ_PX = 2.0;   # only knob — Gaussian σ on the SVD noise component
...
let
    ...
    panels = (
        (1, 1, "80 kVp", "After SVD denoiser", slice_lo),
        (1, 2, "140 kVp", "After SVD denoiser", slice_hi),
    )
...
!!! note "Solid-Water Diagnostic"
    ...
    after the SVD sinogram denoiser landed, the residual bias
